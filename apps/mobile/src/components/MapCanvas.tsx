...
export default function MapCanvas({
  ...
  // Fit the radius circle in view when a place is picked from search.
  useEffect(() => {
  ...
  const emitBBox = async () => {
    ...
    if (!map) return;
    try {
      const bounds = await map.getVisibleBounds(); // [[neLng, neLat], [swLng, swLat]]
      const [ne, sw] = bounds;
      onBBoxChange({
        minLng: sw[0],
        minLat: sw[1],
  ...
  return (
    <View style={StyleSheet.absoluteFill}>
      ...
      >
        ...
        {poi && (
          <MarkerView coordinate={[poi.lng, poi.lat]} anchor={{ x: 0.5, y: 0.5 }}>
        ...
        {listings.map((l) => (
          ...
          >
            <Pressable
              ...
              style={[styles.pin, activeId === l.id && styles.pinActive]}
            >
              <Text
                style={[
                  styles.pinText,
                  activeId === l.id && styles.pinTextActive,
                ]}
              >
                {sizeLabel(l)}
              </Text>
      ...
      {/* Locate control */}
      <Pressable style={styles.locateBtn} onPress={locate} hitSlop={8}>
        <LocateIcon />
      </Pressable>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  pinText: { fontSize: 12, fontWeight: "700", color: colors.brand },
